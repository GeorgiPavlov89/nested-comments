...
import { deleteReplay } from "../services/comments";
import {
  MDBCard,
  MDBCardBody,
  MDBCardImage,
...
function SingleReplay({ content, createdAt, score, username, userImg, replyingTo,replayId,commentId,onDelete }) {
  const dateFormater = new Intl.DateTimeFormat(undefined, {
    dateStyle: "medium",
    timeStyle: "short",
  });

  function handleDeleteReplay() {
    deleteReplay(commentId, replayId) 
        .then((response) => {
          onDelete(replayId)
        })
        .catch((error) => {
              ...
              <div className="flex-grow-1 flex-shrink-1">
                <div>
                  <p className="small mb-0">
                    <span style={{ color: "#5457b6" }}>@{replyingTo} </span>
                    {content}
                  </p>
                  <div className="d-flex justify-content-between">
                    <div className="d-flex align-items-center square bg-light rounded-5">
                      ...
                      <span style={{ color: "#5357B6" }} className="fw-bold">
                        {score}
                      </span>
                      <MDBBtn
                        className="mx-2"
